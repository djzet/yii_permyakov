...
Carousels support swiping left/right on touchscreen devices to move between slides. This can be disabled by setting
the `touch` option to `false`.
...
| `next` | Cycles to the next item. **Returns to the caller before the next item has been shown** (e.g., before
